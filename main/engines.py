...
def train_fn(
    ...
    save_ckps_dir = ".",
    patience = 25,  # Patience untuk early stopping, set ke banyak untuk tanpa early stopping.
):
    print("\nStart Training ...\n" + " = "*16)
    ...
    best_accuracy = 0.0
    no_improve_count = 0  # Counter epoch tanpa peningkatan val_accuracy
    
    for epoch in range(1, num_epochs + 1):
        print("epoch {}/{}".format(epoch, num_epochs) + "\n" + " - "*16)

        with torch.autograd.set_detect_anomaly(True):
            FT.train(), FS.train(), GS.train()
            for images_T, labels_T, domains_T in tqdm.tqdm(train_loaders["train"]):
                ...
                features_T = FT.backbone(images_T.float())
                features_S = FS.backbone(images_S.float())
                ...
                for input, other, in itertools.combinations(range(1, 4), 2):
                    distance = torch.dist(
                        input = torch.mean(images_T[domains_T == input], axis = 0), 
                        other = torch.mean(images_T[domains_T == other], axis = 0), 
                        p = 2, 
                    )
                ...
                loss_GS = F.cross_entropy(FT.classifier(features_S), labels_T) \
                          - torch.minimum(discrepancy - sum(pairwise_distances)/len(pairwise_distances), torch.zeros(1).to(device))
        ...
        with torch.no_grad():
            ...
            for images_T, labels_T, domains_T in tqdm.tqdm(train_loaders["val"]):
                ...
                running_loss += loss.item()*images_T.size(0)
                running_corrects += torch.sum(torch.max(logits, 1)[1] == labels_T.data).item()
        val_loss = running_loss/len(train_loaders["val"].dataset)
        val_accuracy = running_corrects/len(train_loaders["val"].dataset)
        print('running loss =', running_loss)
        print("{:<8} - loss:{:.4f}, accuracy:{:.4f}".format("val", val_loss, val_accuracy))
        ...
        torch.save(FT, "{}/FT-all.ptl".format(save_ckps_dir))
        torch.save(GS, "{}/GS-all.ptl".format(save_ckps_dir))
